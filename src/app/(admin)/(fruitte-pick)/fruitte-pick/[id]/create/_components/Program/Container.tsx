...
const ProgramContainer = () => {
  const { program, setProgram } = useFruittePickIntroCreateStore();
  ...
  const handleDrop = (index: number) => {
    ...
    const [draggedItem] = updated.splice(draggedIndex, 1); // 드래그한 항목 제거
    updated.splice(index, 0, draggedItem); // 새로운 위치에 추가

    setProgram(updated); // 상태 업데이트

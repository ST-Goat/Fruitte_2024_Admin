...
const TicketContainer = () => {
  const { ticket, setTicket } = useFruittePickIntroCreateStore();
  ...
  const handleDrop = (index: number) => {
    ...
    const [draggedItem] = updatedTickets.splice(draggedIndex, 1); // 드래그한 항목 제거
    updatedTickets.splice(index, 0, draggedItem); // 새로운 위치에 추가

    setTicket(updatedTickets); // 상태 업데이트

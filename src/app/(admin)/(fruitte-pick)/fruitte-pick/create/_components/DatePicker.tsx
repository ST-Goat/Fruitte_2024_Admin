...
const DatePicker = () => {
  const { exposedTime, setExposedTime } = useFruittePickCreateStore();
  ...
  useEffect(() => {
    if (datePickerRef.current) {
      ...
      flatpickrInstance.current = flatpickr(datePickerRef.current, {
        ...
        defaultDate: exposedTime,
        onChange: (selectedDates) => {
          if (selectedDates.length > 0) {
            setExposedTime(selectedDates[0]); // Zustand 상태 업데이트
          }
        },
      });
    }

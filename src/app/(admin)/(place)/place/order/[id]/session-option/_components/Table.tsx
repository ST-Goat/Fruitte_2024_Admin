...
const Table = () => {
  ...
  const columns: Column<i.SessionOptionTable>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ value }) =>
          moment(value, "YYYY-MM-DD").format("YYYY-MM-DD(ddd)"),
      },
      {
        Header: "시간",
        accessor: "sessionTime",
        Cell: ({ value }) => moment(value, "HH:mm:ss").format("HH:mm"),
      },
      {
        Header: "공개순위",
        accessor: "step",
  ...
  // selectedRow 업데이트 함수
  useEffect(() => {
    setSelectedRow(selectedFlatRows.map((row) => row.original)); // selectedFlatRows에서 원본 데이터를 추출하여 상태에 저장
